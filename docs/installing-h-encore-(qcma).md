...
1. Click the `Get Homebrews` button, and save the `.zip` file onto your computer. Unzip the files so they are ready to be moved.
...
1. Navigate to `HENkaku Settings`.
    * If HENkaku Settings is missing, select the `Reset taiHEN config.txt` option in the h-encore bootstrap menu, then try again.
